...
for UnwrapIdx = 1:NumUnwraps 
    if isfield(ProcessUnwrap{UnwrapIdx},'PMU')
        ...
        for PMUidx = 1:NumPMU
            ...
            TempIdx = find(strcmp(ProcessUnwrap{UnwrapIdx}.PMU{PMUidx}.Name, {PMU.PMU_Name}));
            if ~isempty(TempIdx)
                PMUstructIdx(PMUidx) = TempIdx;
            else
                warning(['PMU ' ProcessUnwrap{UnwrapIdx}.PMU{PMUidx}.Name ' was not found.']);
            end
    ...
    for PMUidx = 1:NumPMU
        ...
        [PMU(PMUstructIdx(PMUidx)), FinalAngles{UnwrapIdx}{PMUidx}.Signals] = UnwrapAngle(PMU(PMUstructIdx(PMUidx)),PMUchans(PMUidx).ChansToFilt, PastAngles{UnwrapIdx}{PMUidx}.Signals);
    end    
end