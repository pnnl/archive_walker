...
% In some data, the JSIS-CSV format hasn't been followed exactly. These
% lines allow for slight deviations.
signalUnitStr = strrep(signalUnitStr,'Deg','DEG');
signalUnitStr = strrep(signalUnitStr,'Amps','A');
...
if(timeFormat == 1)
    ...
    timeStr = datestr(timeNum, 'yyyy-mm-dd HH:MM:SS.FFF');
elseif(timeFormat == 0)
    % time are in seconds
    try
...
try
    ...
    % In some cases, the JSIS-CSV format hasn't been followed exactly and
    % bad data is indicated with 'Bad' rather than with NaN. Files with 'Bad' in them
    % get converted to cell arrays instead of matrices in the previous
    % line. If this happens, attempt to convert from cell to matrix. This
    % probably will fail, leading to the catch statement. Inside the catch,
    % 'Bad' is converted to NaN.
