...
if isfield(Parameters,'FrequencyTolerance')
    ...
    switch WindowType
        case 'rectwin'
            FrequencyTolerance = 2*fs/AnalysisLength; % fs * 4*pi/AnalysisLength / (2*pi);
        case 'bartlett'
            FrequencyTolerance = 4*fs/AnalysisLength; % fs * 8*pi/AnalysisLength / (2*pi);
        case 'hann'
            FrequencyTolerance = 4*fs/AnalysisLength; % fs * 8*pi/AnalysisLength / (2*pi);
        case 'hamming'
            FrequencyTolerance = 4*fs/AnalysisLength; % fs * 8*pi/AnalysisLength / (2*pi);
        case 'blackman'
            FrequencyTolerance = 6*fs/AnalysisLength; % fs * 12*pi/AnalysisLength / (2*pi);
    end
