...
VV=(1/sqrt(3))*Vm.*exp(1i*(pi/180)*(Va-30));
...
for k=1:length(V1)
    ...
    if ~InitializedFlag
        if ~isnan(sum([V1(k) I1(k) P1(k) Q1(k) Zl_1(k)]))
            ...
            Eth_max1=V1(k)*(cos(theta1(k))+1i*sin(theta1(k)))*cos(theta_aux1)/cos(beta_aux1);
            Eth1(k)=0.95*(Eth_max1+V1(k)*(cos(theta1(k))+1i*sin(theta1(k))))/2;
    ...
    if abs(V1(k)/(I1(k))-V1past/(I1past)) < 0.01*(V1past/(I1past))
        if (Zl_1(k)-Zl_1past)>0.00005
            zz2_1=((Eth1_1past-V1(k))/(I1(k)))-((Eth1_1past-V1past)/(I1past));
            if(zz2_1>0)
                Eth_aux2_11=k1*Eth1_1past;
                Eth1(k)=Eth1past-Eth_aux2_11;
            else
                Eth_aux2_11=k1*Eth1_1past;
                Eth1(k)=Eth1past+Eth_aux2_11;
            end
        elseif (Zl_1(k)-Zl_1past)<-0.00005
            zz2_1=((Eth1_1past-V1(k))/(I1(k)))-((Eth1_1past-V1past)/(I1past));
            if(zz2_1<0)
                Eth_aux2_11=k1*Eth1_1past;
                Eth1(k)=Eth1past-Eth_aux2_11;
    ...
    angle_Z(k) = angle(res1(1)+1i*res1(2));
