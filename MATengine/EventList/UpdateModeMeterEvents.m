...
% % EventList = UpdateModeMeterEvents(DetectionResults, AdditionalOutput, EventList, Params, AlarmParams);
% UpdatedEventList = [];
% RevisitOccurrence = zeros(0,2);
% for ModeIdx = 1:length(DetectionResults)
%     % If no FOs were detected, move to next detection result
%     if isempty(length(DetectionResults(ModeIdx).LowDampingRatio))
%         EventList(ModeIdx) = [];
%         continue
%     end
%     
%     if ~isfield(EventList(ModeIdx),'ChannelsWithLowDampedMode')
%         EventList(ModeIdx).ID = AssignEventID();
%         EventList(ModeIdx).ModeOfInterest = DetectionResults(ModeIdx).ModeOfInterest;
%         EventList(ModeIdx).Start = AdditionalOutput(ModeIdx).Tstart;
%         EventList(ModeIdx).End = AdditionalOutput(ModeIdx).Tend;
%         EventList(ModeIdx).OccurrenceID = {AssignEventID()};
%         EventList(ModeIdx).ChannelsWithLowDampedMode = DetectionResults(ModeIdx).ChannelsWithLowDampedMode;
%         EventList(ModeIdx).PMUWithLowDampedMode = DetectionResults(ModeIdx).PMUWithLowDampedMode;
%         for ChanIdx = 1:length(DetectionResults(ModeIdx).ChannelsWithLowDampedMode)
%             EventList(ModeIdx).MethodsWithLowDampedMode{ChanIdx} = DetectionResults(ModeIdx).MethodsWithLowDampedMode{ChanIdx};
%             EventList(ModeIdx).LowDampingRatio{ChanIdx} = DetectionResults(ModeIdx).LowDampingRatio{ChanIdx};            
%             EventList(ModeIdx).FreqLowDampingRatio{ChanIdx} = DetectionResults(ModeIdx).FreqLowDampingRatio{ChanIdx};            
%         end
%     else
...
%             end
%             FrequenciesToAdd{EventIdx} = [FrequenciesToAdd{EventIdx} DetectionResults(ModeIdx).Frequency(FreqIdx)];
%             
...
%             % listed, add it.
%             for ChanIdx = 1:length(DetectionResults(ModeIdx).Channel)
%                 ChanLoc = strcmp(DetectionResults(ModeIdx).Channel{ChanIdx}, EventList(EventIdx).Channel{end}) & strcmp(DetectionResults(ModeIdx).PMU{ChanIdx}, EventList(EventIdx).PMU{end});
%                 if sum(ChanLoc) == 0
%                     % This channel hasn't been added yet, so add it
%                     EventList(EventIdx).Channel{end} = [EventList(EventIdx).Channel{end}, {DetectionResults(ModeIdx).Channel{ChanIdx}}];
%                     EventList(EventIdx).PMU{end} = [EventList(EventIdx).PMU{end}, {DetectionResults(ModeIdx).PMU{ChanIdx}}];
%                     EventList(EventIdx).Unit{end} = [EventList(EventIdx).Unit{end}, {DetectionResults(ModeIdx).Unit{ChanIdx}}];
%                     
%                     if isfield(DetectionResults(ModeIdx),'Amplitude')
%                         % Periodogram
%                         EventList(EventIdx).Amplitude{end} = [EventList(EventIdx).Amplitude{end}, DetectionResults(ModeIdx).Amplitude(FreqIdx,ChanIdx)];
%                         EventList(EventIdx).SNR{end} = [EventList(EventIdx).SNR{end}, DetectionResults(ModeIdx).SNR(FreqIdx,ChanIdx)];
%                         EventList(EventIdx).Coherence{end} = [EventList(EventIdx).Coherence{end}, NaN];
%                     else
%                         % Spectral coherence
%                         EventList(EventIdx).Coherence{end} = [EventList(EventIdx).Coherence{end}, DetectionResults(ModeIdx).Coherence(FreqIdx,ChanIdx)];
...
%                     if isfield(DetectionResults(ModeIdx),'Amplitude')
%                         % Periodogram
%                         if (isnan(EventList(EventIdx).Amplitude{end}(ChanLoc)) || (EventList(EventIdx).Amplitude{end}(ChanLoc) < DetectionResults(ModeIdx).Amplitude(FreqIdx,ChanIdx)))
%                             EventList(EventIdx).Amplitude{end}(ChanLoc) = DetectionResults(ModeIdx).Amplitude(FreqIdx,ChanIdx);
%                         end
%                         if (isnan(EventList(EventIdx).SNR{end}(ChanLoc)) || (EventList(EventIdx).SNR{end}(ChanLoc) < DetectionResults(ModeIdx).SNR(FreqIdx,ChanIdx)))
%                             EventList(EventIdx).SNR{end}(ChanLoc) = DetectionResults(ModeIdx).SNR(FreqIdx,ChanIdx);
%                         end
%                     else
%                         % Spectral coherence
%                         if (isnan(EventList(EventIdx).Coherence{end}(ChanLoc)) || (EventList(EventIdx).Coherence{end}(ChanLoc) < DetectionResults(ModeIdx).Coherence(FreqIdx,ChanIdx)))
%                             EventList(EventIdx).Coherence{end}(ChanLoc) = DetectionResults(ModeIdx).Coherence(FreqIdx,ChanIdx);
%                         end
%                     end
%                 end
%             end
...
%             EventList(EventIdx).Frequency = [EventList(EventIdx).Frequency, DetectionResults(ModeIdx).Frequency(FreqIdx)];
...
%             %                         EventList(EventIdx).Channel = [EventList(EventIdx).Channel, DetectionResults(DetIdx).Channel];
%             %                     end
%             EventList(EventIdx).Channel = [EventList(EventIdx).Channel, {DetectionResults(ModeIdx).Channel}];
%             EventList(EventIdx).PMU = [EventList(EventIdx).PMU, {DetectionResults(ModeIdx).PMU}];
%             EventList(EventIdx).Unit = [EventList(EventIdx).Unit, {DetectionResults(ModeIdx).Unit}];
%             
%             if isfield(DetectionResults(ModeIdx),'Amplitude')
%                 % Periodogram
%                 EventList(EventIdx).Amplitude = [EventList(EventIdx).Amplitude DetectionResults(ModeIdx).Amplitude(FreqIdx,:)];
%                 EventList(EventIdx).SNR = [EventList(EventIdx).SNR DetectionResults(ModeIdx).SNR(FreqIdx,:)];
%                 EventList(EventIdx).Coherence = [EventList(EventIdx).Coherence NaN*ones(1,length(DetectionResults(ModeIdx).Channel))];
%             else
%                 % Spectral Coherence
%                 EventList(EventIdx).Coherence = [EventList(EventIdx).Coherence DetectionResults(ModeIdx).Coherence(FreqIdx,:)];
%                 EventList(EventIdx).Amplitude = [EventList(EventIdx).Amplitude NaN*ones(1,length(DetectionResults(ModeIdx).Channel))];
%                 EventList(EventIdx).SNR = [EventList(EventIdx).SNR NaN*ones(1,length(DetectionResults(ModeIdx).Channel))];
%             end
%         end
%     end
%     
...
%     EventList(1).Frequency = DetectionResults(ModeIdx).Frequency(FreqIdx);
...
%     %                 EventList(1).Channel = DetectionResults(DetIdx).Channel;
%     %             end
%     EventList(1).Channel = {DetectionResults(ModeIdx).Channel};
%     EventList(1).PMU = {DetectionResults(ModeIdx).PMU};
%     EventList(1).Unit = {DetectionResults(ModeIdx).Unit};
%     
%     if isfield(DetectionResults(ModeIdx),'Amplitude')
%         % Periodogram
%         EventList(1).Amplitude = {DetectionResults(ModeIdx).Amplitude(FreqIdx,:)};
%         EventList(1).SNR = {DetectionResults(ModeIdx).SNR(FreqIdx,:)};
%         EventList(1).Coherence = {NaN*ones(1,length(DetectionResults(ModeIdx).Channel))};
%     else
%         % Spectral Coherence
%         EventList(1).Coherence = {DetectionResults(ModeIdx).Coherence(FreqIdx,:)};
%         EventList(1).Amplitude = {NaN*ones(1,length(DetectionResults(ModeIdx).Channel))};
%         EventList(1).SNR = {NaN*ones(1,length(DetectionResults(ModeIdx).Channel))};
%     end
% end
% end
% 
