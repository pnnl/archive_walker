function EventList = UpdateTheveninOverEvents(EventList,PathEventXML,DetectorXML,EventType,DetectorIndex,TimeStamp)
...
if NumDetectors == 1
    if isfield(DetectorXML.(EventType),'EventMergeWindow')
        EventMergeWindow = str2num(DetectorXML.(EventType).EventMergeWindow);
    else
        EventMergeWindow = 0;
    end
else
    if isfield(DetectorXML.(EventType){DetectorIndex},'EventMergeWindow')
        EventMergeWindow = str2num(DetectorXML.(EventType){DetectorIndex}.EventMergeWindow);
    else
        EventMergeWindow = 0;
    end
end
...
EventEndLimit = TimeStamp - EventMergeWindow/(60*60*24);
...
for EventIdx = OverEvents
    st = EventList.([EventType '_' DetectorIndex])(EventIdx).Start;
    en = EventList.([EventType '_' DetectorIndex])(EventIdx).End;
