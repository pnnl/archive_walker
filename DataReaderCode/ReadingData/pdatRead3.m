...
function KeepIdxOut = GetKeepIdx(numChan,ChanArray,ChanName)
% This block gets all of the names for the channels. The
% somewhat complicated code is needed to get the name field
% from each structure in the cell array.
nn = cell(1,numChan);
[nn{:}] = deal('name');
AvailableNames = cellfun(@getfield,ChanArray,nn,'UniformOutput',false);

ThisChan = find(strcmp(ChanName, AvailableNames));
if length(ThisChan) == 1
    KeepIdxOut = ThisChan;
elseif length(ThisChan) > 1
    error('Multiple channels in the file have the same name.');
else
    KeepIdxOut = [];
end
end