...
NumDetectors = length(DetectorXML.Configuration.(EventType));
% Find Hist, the number of seconds of data from previous files
% that this detector utilizes (new events can be added in this
% time window)
if NumDetectors == 1
    Hist = str2double(DetectorXML.Configuration.(EventType).AnalysisWindow);
else
    Hist = str2double(DetectorXML.Configuration.(EventType){DetectorIndex}.AnalysisWindow);
end
