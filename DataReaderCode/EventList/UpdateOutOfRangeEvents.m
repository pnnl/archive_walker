function EventList = UpdateOutOfRangeEvents(DetectionResults, ~, EventList, ~)
...
elseif isfield(DetectionResults,'RingStart')
    ...
    for ChanIdx = 1:length(DetectionResults)
        % RingStart is set to NaN when the channel is discarded. Ignore
        % these channels.
        if ~isnan(DetectionResults(ChanIdx).RingStart)
